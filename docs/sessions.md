...
Capabilities maps the transport-backed command names to `available`, `unavailable`, `unsupported` or `unknown`. Only `available` may be used to dispatch an action by the control implementation. Unknown/exceptional feature observations stay `unknown`. Keyboard type additionally requires confirmed keyboard focus; otherwise it is unavailable. Status, capabilities and core controls are implemented; app/keyboard CLI commands still return `FEATURE_UNAVAILABLE` until issue 007 lands. This table describes transport capability, not completion of those CLI implementations.
