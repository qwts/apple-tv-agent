...
The separate `apple-tv-screen` executable is a read-only prerequisite for optional LG observation. It supports **discover** and **inspect**, plus [trusted pairing and local credential management](lg-pairing.md). [One-shot capture and HDMI association](lg-capture.md) are optional follow-up commands under [issue 18](https://github.com/qwts/apple-tv-agent/issues/18). The Apple TV CLI is unchanged. The discovery and inspection commands documented here do not read the native vault, save registrations or send remote controls.
