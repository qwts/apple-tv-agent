...
A vault deletion failure retains the registry record/default and reports removed/failed protocol labels. Unlock the vault and retry with the returned UUID. Repeating a successful deletion with the same explicit UUID is idempotent, including after the registry record is gone. An alias no longer resolves after successful deletion. If credentials were removed but the registry update failed, the error reports that distinction and permits the same UUID retry. Keep the registry intact until coordinated deletion succeeds.
