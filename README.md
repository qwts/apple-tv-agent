...
**Status: discovery, device registry and native-vault pairing, status and capabilities implemented; core navigation/playback/power/volume implemented; apps, keyboard, diagnostics and agent skill pending.** Follow the [package setup](docs/cli-contract.md), [registry guide](docs/registry.md) and [interactive pairing guide](docs/pairing.md), and [status/capabilities guide](docs/sessions.md) and [controls guide](docs/controls.md). Pairing runs in a local terminal with hidden PIN input; verified credentials remain in the native vault. Hardware support evidence and remaining Windows gates are tracked in [compatibility](docs/compatibility.md).
