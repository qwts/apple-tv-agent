...
Capabilities depend on the TV, active app and audio/display setup. The baseline CLI does not expose screenshots, screen reading, Siri, purchases, arbitrary deep links or internet remote access. An optional LG provider has [HDMI screenshot feasibility evidence](docs/lg-screen-capture-spike.md) and a [follow-up implementation plan](issues/011-lg-screen-observation.md); capture is provided by the optional local helper. The experimental [LG discovery helper](docs/lg-discovery.md) can discover candidates and inspect an untrusted TLS fingerprint; it supports [trusted LG pairing](docs/lg-pairing.md) and [one-shot capture with explicit HDMI association](docs/lg-capture.md).
