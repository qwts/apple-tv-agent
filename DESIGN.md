...
Integration tests use mocked pyatv APIs plus real native credential backends only in explicitly opted-in, isolated test namespaces. Test cleanup and partial failures. The Python 3.12/3.14 package matrix runs on macOS and Windows; the native bundle matrix also runs on Linux and both Mac architectures, with no TV or credentials required, and builds/installs the package in a fresh environment. Validate skill links and frontmatter, and run realistic agent scenarios against a fake CLI.

Hardware validation must record host OS/architecture, Python/pyatv versions, Apple TV model/tvOS, network topology and audio route. From all three target host OSes (both Mac architectures for their release assets): discover, pair, restart and reconnect, inspect status, pause/play, navigate, list/launch apps, test power/volume/keyboard when available, invalidate credentials, interrupt connectivity, and exercise multiple-TV ambiguity when hardware exists. Record pass/fail/unsupported/not-tested per case. Unsupported is acceptable only with the expected structured error; untested is not a pass. No stable macOS/Windows/Linux release claim until every target baseline passes; record native validation for the exact version in release-gates.json.

## Risks and decisions to close

The merged issue 001 probe provides API and dependency-install evidence for implementing the provisional v1 contract. Native Windows 11 discovery, pairing and keyring persistence remain issue 001 release gates; implementing the contract does not claim those checks passed. tvOS updates can change protocol behavior; keep compatibility evidence and a pinned dependency with deliberate updates. Firewall, VPN, multicast filtering and guest-network isolation can prevent discovery; diagnostics should explain observations without modifying network settings. Pairing UI may depend on TV configuration; refer users to upstream troubleshooting without silently relaxing access settings. [Source](https://pyatv.dev/support/troubleshooting/)
...
## Optional display observation boundary

LG screenshot observation is a separate opt-in follow-up outside the baseline Apple TV CLI. Its [provider contract](docs/observation-contract.md) defines explicit Apple TV/LG/HDMI association, independent versioned image metadata, trust-before-credential reuse, and context eligibility. Models, discovery, trusted pairing, explicit bindings, optional one-shot capture and skill guidance are implemented under issue 18; native Windows hardware validation remains open. A screen observation never changes a baseline action outcome to confirmed automatically.

## Versioned distribution

[Issue 30](https://github.com/qwts/apple-tv-agent/issues/30) adds native onedir bundles with embedded Python, both CLI entry points, the portable skill and optional image decoder. Tag/version equality, complete asset checks and all archive smoke tests gate draft GitHub Release creation. Stable tags additionally require recorded native validation for every target. PR/manual builds cannot publish; the workflow never publishes a draft automatically. See [pipeline maintenance](docs/release-pipeline.md) and [downloads](docs/downloads.md). Publisher signing/notarization and native Linux/Windows hardware evidence remain open; unsigned previews must say so.
