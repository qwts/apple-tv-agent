...
LG screenshot observation is a separate opt-in follow-up outside the baseline Apple TV CLI. Its [provider contract](docs/observation-contract.md) defines explicit Apple TV/LG/HDMI association, independent versioned image metadata, trust-before-credential reuse, and context eligibility. Models and an asynchronous interface exist; production LG discovery, pairing, capture and skill integration remain issue 18 work. A screen observation never changes a baseline action outcome to confirmed automatically.
